...
class navigationControl(Node):
    ...
    def listener_callback(self,msg):  ##Trabaja con la Occupancy Grid obtenida de 'map'. 
        if self.flag == 1:
            ...
            path, self.explored_cells = dijkstra(self, data,(row,column),(rowH,columnH)) #Busqueda de ruta con Dijkstra
...
def main(args=None):
    ...
    with open(file_path, mode="a", newline="") as file:  
        ...
        if not file_exists:
            writer.writerow(["Simulación","X Deseada", "Y Deseada", "Tiempo Path Building", "Tiempo de Trayectoria", "Tiempo Total", "Distancia Total","Numero de Giros","Celdas Exploradas"])
