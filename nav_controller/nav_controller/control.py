...
class navigationControl(Node):
    ...
    def follow_path(self):
        column = int((self.x- self.originX)/self.resolution) #x,y koordinatlarından costmap indislerine çevirme
        row = int((self.y- self.originY)/self.resolution) #x,y koordinatlarından costmap indislerine çevirme
        columnH = int((self.goal[0]- self.originX)/self.resolution)#x,y koordinatlarından costmap indislerine çevirme
        rowH = int((self.goal[1]- self.originY)/self.resolution)#x,y koordinatlarından costmap indislerine çevirme
        data = costmap(self.map_data,self.width,self.height,self.resolution) #costmap düzenleme
        data[row][column] = 0 #robot konumu
        data[data < 0] = 1 
        data[data > 5] = 1 
        path = astar(data,(row,column),(rowH,columnH)) #astar algoritması ile yol bulma
        path = [(p[1]*self.resolution+self.originX,p[0]*self.resolution+self.originY) for p in path] #x,y koordinatlarına çevirme
